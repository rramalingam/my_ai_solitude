...
# exploring usage of LLM for generating personalised emails to customers/individuals
#
...
def get_ai_response (prompts, temprature=0):
    # response = client.models.generate_content(
    # model = "gemini-2.5-flash", contents=prompts, config={'temperature':temperature})
    response = client.models.generate_content(
    model = "gemini-2.5-flash", contents=prompts, config=types.GenerateContentConfig(temperature=temprature))
    return response
...
res = get_ai_response(prompt_reply, temprature=2)
print(res.text)
